...
const TerminalEngine = () => {
  ...
  useEffect(() => {
    ...
    if (typing) {
      ...
      if (charIndex < currentCmd.length) {
        ...
        // Finished typing command -> hit 'Enter' after a brief delay
        timeout = setTimeout(() => {
          setTyping(false);
      ...
      const printLine = () => {
        if (lineIndex < outputLines.length) {
          ...
          lineIndex++;
          // Simulate latency in terminal output
          timeout = setTimeout(printLine, Math.random() * 300 + 100);
        } else {
          // Finished execution -> wait, then load next command
          timeout = setTimeout(() => {
            setCmdIndex(prev => (prev + 1) % COMMANDS.length);
            setCharIndex(0);
            setTyping(true);
            
            // Garbage collect history to prevent infinite DOM leak
  ...
  return (
    <div className="fixed bottom-6 left-6 z-40 w-[350px] md:w-[480px] transition-all duration-300 pointer-events-auto">
      <div className="bg-[#080e1a]/80 backdrop-blur-xl border border-[#424855]/40 rounded-lg overflow-hidden flex flex-col shadow-[0_10px_40px_rgba(255,141,134,0.1)] transition-transform hover:-translate-y-2 duration-300">
        
        {/* MacOS Style Terminal Header */}
        <div className="bg-[#131a28]/90 px-4 py-2.5 flex items-center border-b border-[#424855]/40 relative">
          ...
          <span className="w-full text-center text-[11px] text-[#a6abbb] font-mono tracking-wider font-semibold">
            abhijit@obsidian-core:~
          </span>
        </div>
        
        {/* Scrollable Output Content */}
        <div ref={scrollRef} className="p-4 h-[220px] overflow-y-auto font-mono text-xs md:text-sm flex flex-col gap-1.5 text-[#e0e5f6] terminal-scroll">
          {history.map((line, i) => (
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .terminal-scroll::-webkit-scrollbar-thumb {
          background: rgba(66, 72, 85, 0.5);
          border-radius: 4px;
        }
        .terminal-scroll::-webkit-scrollbar-thumb:hover {
          background: rgba(144, 147, 255, 0.5);
        }
      `}} />
    </div>
  );
